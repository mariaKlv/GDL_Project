...
class GEmbedNet(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        if self.pooling:
            start_pooling = torch.cuda.Event(enable_timing=True)
            end_pooling = torch.cuda.Event(enable_timing=True)

            start_pooling.record()
            x, edge_index = self.pool(x, edge_index)
            end_pooling.record()
            torch.cuda.synchronize()
            pool_time = start_pooling.elapsed_time(end_pooling)
...
def run():
    ...
    for train_val_idxs, test_idxs in kfold.split(np.zeros(len(dataset.data.y)), dataset.data.y):
        ...
        print("Training for fold {0} took {1}s".format(current_fold, fold_training_time))
